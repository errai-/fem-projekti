...
zdata=[];
for datum=data
    zdata=[zdata datum.error./(datum.k.^1.0)];
end
tri = delaunay(xdata, ydata);
trisurf(tri, xdata, ydata, zdata);
set(gca,'xscale','log','yscale','log','zscale','log');
xlabel('h (average)'); ylabel('k'); zlabel('total error/k');

pause;

xdata=[];
for datum=data
    xdata=[xdata datum.h_avg*(datum.k.^1.0)];
end
ydata=[data.k];
zdata=[];
for datum=data
...
trisurf(tri, xdata, ydata, zdata);
set(gca,'xscale','log','yscale','log','zscale','log');
xlabel('h (average) *k'); ylabel('k'); zlabel('total error/k');
...
trisurf(tri, xdata, ydata, zdata);
set(gca,'xscale','log','yscale','log','zscale','log');
xlabel('h (average) *k^{1.5}'); ylabel('k'); zlabel('total error/k');
...
trisurf(tri, xdata, ydata, zdata);
set(gca,'xscale','log','yscale','log','zscale','log');
xlabel('number of nodes'); ylabel('k^3'); zlabel('total error/k');